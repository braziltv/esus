  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-40 shrink-0 transition-opacity duration-300 ${isAnnouncing ? 'opacity-30' : 'opacity-100'}`}>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex items-stretch h-8 xs:h-10 sm:h-12 md:h-14 lg:h-16 xl:h-18 2xl:h-20 3xl:h-24 qhd:h-28 4k:h-40">
          ...
          <div className="flex-1 bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 overflow-hidden flex items-center relative">
            {/* Top red accent line */}
            <div className="absolute top-0 left-0 right-0 h-[2px] sm:h-[3px] lg:h-1 qhd:h-1.5 4k:h-2 bg-gradient-to-r from-red-600 via-red-500 to-red-600" />
            
            {/* Gradient fade edges */}
            <div className="absolute left-0 top-0 bottom-0 w-4 xs:w-6 sm:w-8 md:w-12 lg:w-16 xl:w-20 qhd:w-28 4k:w-40 bg-gradient-to-r from-gray-900 to-transparent z-10" />
            <div className="absolute right-0 top-0 bottom-0 w-4 xs:w-6 sm:w-8 md:w-12 lg:w-16 xl:w-20 qhd:w-28 4k:w-40 bg-gradient-to-l from-gray-900 to-transparent z-10" />
            
            {/* Scrolling content */}
            <div className="animate-marquee whitespace-nowrap inline-flex py-1 qhd:py-2 4k:py-3">
              {items.map((item, index) => (
                <span key={index} className="mx-2 xs:mx-3 sm:mx-4 md:mx-5 lg:mx-6 xl:mx-8 qhd:mx-10 4k:mx-16 inline-flex items-center gap-1 xs:gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 qhd:gap-5 4k:gap-8 font-semibold tracking-wide text-[10px] xs:text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-2xl 3xl:text-3xl qhd:text-4xl 4k:text-6xl" style={{ fontFamily: 'Poppins, system-ui, sans-serif' }}>
                  <span className={`px-1.5 xs:px-2 sm:px-2.5 md:px-3 lg:px-4 xl:px-5 qhd:px-6 4k:px-10 py-0.5 xs:py-1 sm:py-1.5 md:py-2 qhd:py-2.5 4k:py-4 rounded-sm sm:rounded qhd:rounded-lg 4k:rounded-xl text-[8px] xs:text-[9px] sm:text-[10px] md:text-xs lg:text-sm xl:text-base 2xl:text-lg 3xl:text-xl qhd:text-2xl 4k:text-4xl font-bold inline-flex items-center gap-1 qhd:gap-2 4k:gap-3 ${getSourceStyle(item.source)} ${item.source === '📢 Informativo' ? 'animate-pulse' : ''}`}>
                    {item.source === 'Créditos' ? '⭐' : item.source === '📢 Informativo' ? (
                      <>
                        <Megaphone className="w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-5 lg:h-5 xl:w-6 xl:h-6 qhd:w-8 qhd:h-8 4k:w-12 4k:h-12 inline animate-bounce" />
                        <span className="hidden xs:inline">INFORMATIVO</span>
                      </>
                    ) : item.source}
                  </span>
                  <span className={`drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] qhd:drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] 4k:drop-shadow-[0_3px_6px_rgba(0,0,0,0.8)] ${
                  ...
                  <span className="text-red-500 mx-2 xs:mx-3 sm:mx-4 qhd:mx-6 4k:mx-10 text-xs sm:text-base md:text-lg lg:text-xl xl:text-2xl qhd:text-3xl 4k:text-5xl">▸</span>
                </span>
              ))}
              {/* Duplicate for seamless loop */}
              {items.map((item, index) => (
                <span key={`dup-${index}`} className="mx-2 xs:mx-3 sm:mx-4 md:mx-5 lg:mx-6 xl:mx-8 qhd:mx-10 4k:mx-16 inline-flex items-center gap-1 xs:gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 qhd:gap-5 4k:gap-8 font-semibold tracking-wide text-[10px] xs:text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-2xl 3xl:text-3xl qhd:text-4xl 4k:text-6xl" style={{ fontFamily: 'Poppins, system-ui, sans-serif' }}>
                  <span className={`px-1.5 xs:px-2 sm:px-2.5 md:px-3 lg:px-4 xl:px-5 qhd:px-6 4k:px-10 py-0.5 xs:py-1 sm:py-1.5 md:py-2 qhd:py-2.5 4k:py-4 rounded-sm sm:rounded qhd:rounded-lg 4k:rounded-xl text-[8px] xs:text-[9px] sm:text-[10px] md:text-xs lg:text-sm xl:text-base 2xl:text-lg 3xl:text-xl qhd:text-2xl 4k:text-4xl font-bold inline-flex items-center gap-1 qhd:gap-2 4k:gap-3 ${getSourceStyle(item.source)} ${item.source === '📢 Informativo' ? 'animate-pulse' : ''}`}>
                    {item.source === 'Créditos' ? '⭐' : item.source === '📢 Informativo' ? (
                      <>
                        <Megaphone className="w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-5 lg:h-5 xl:w-6 xl:h-6 qhd:w-8 qhd:h-8 4k:w-12 4k:h-12 inline animate-bounce" />
